...
The qubenet Service is a Node.js interface to [qubenet Core](https://github.com/QubeNet/qubenet-core) for querying information about the qubenet block chain. It will manage starting and stopping `qubenetd` or connect to several running `qubenetd` processes. It uses an instance of a qubenet with additional indexes enabled for querying information about addresses and blocks. Results are cached for performance and there are several additional API methods added for common queries.

## Configuration

The default configuration will include a "spawn" configuration in "qubenetd". This defines the location of the block chain database and the location of the `qubenetd` daemon executable. The below configuration points to a local clone of `qubenet`, and will start `qubenetd` automatically with your Node.js application.
...
It's also possible to connect to separately managed `qubenetd` processes with round-robin quering, for example:
